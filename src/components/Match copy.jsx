...
import { getBest11, getFantaRating, getFlagFromFifaCode } from '../logic/getBest11';
...
const Match = ({ selectedMatch, setTab }) => {
  ...
  const renderSquad = (squad, label, signColor) => (
    <div className="zw-card" style={{ padding: '20px', marginBottom: '16px', borderLeftColor: signColor }}>
      <h3 style={{ fontFamily: "'Signika', sans-serif", color: 'var(--gold)', marginBottom: '14px', fontSize: '16px', letterSpacing: '0.12em' }}>
        {label}
      </h3>
      {['GK', 'DF', 'MF', 'FW'].map((pos) => {
...
{players.map((p, i) => {
                  const { flag, name } = getFlagFromFifaCode(p.fifa_code);
                  return (
                  <div
                    key={i}
                    style={{
